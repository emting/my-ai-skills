...
來源備份標示的可用工具：`google`。此提示不等於已授權連接或執行；仍須依專案整合邊界與人工核准規則操作。
...
此技能由 `docs/sources/Skills_Full_Configurations_Backup_20260818.md` 的第 23 項正規化而來（原始行號 704–755）。來源內容已補上本 repository 的輸入、輸出、權限、安全與停止契約。
